(function () {
  ...
  const CONFIG = {
    gravity: 0.7,
    jumpPower: 15,
    doubleJump: true,          // ← 2段ジャンプON
    speedStart: 6,
    speedMax: 15.5,
    speedAccel: 0.0010,

    spawnObstacleEvery: 90,
    spawnHoleEvery: 220,
    spawnCoinEvery: 120,
    spawnEnemyEvery: 260,
    ...
    enemySpeedPlus: 1.2,
    enemyDamage: 1,
    iFrames: 60,               // 被弾後の無敵(約1秒)
    enemyHitboxShrink: 0.22,   // ← 当たり判定ゆるく（20%縮小）

    livesStart: 3,
    playerScaleVW: 0.18,       // プレイヤーの画面幅比
    hudWidth: 200              // HUD幅（右上）
  };

  // ===== 画像パス =====
  const ASSETS = {
    player: './走り.PNG',        // 左向き写真
    enemy:  './追いかける.PNG',  // 熊（反転しない）
    home:   './ホーム画面.PNG',
    over:   './襲われた.PNG'
  };
  ...
  const padRight = ()=> parseInt(getComputedStyle(canvas).paddingRight||'0',10) || 0;
  ...
  // プレイヤー
  const player = {
    w:110,h:85,x:0,y:0,vy:0,onGround:true,jumps:2,
    reset(){
    ...
    update(overHole){
      ...
      if(this.onGround){
        ...
        if(stepSE && now-lastStep>170){ play(stepSE); lastStep=now; }
      }
    },
    draw(){
  ...
  // エンティティ
  const obstacles=[], holes=[], coins=[], enemies=[];
  ...
  function updateWorld(){
    speed=Math.min(CONFIG.speedMax, speed+CONFIG.speedAccel); perf++;
  ...
  function shrinkBox(box, ratio){ // 当たり判定緩和
    const mx = box.w*ratio, my = box.h*ratio;
  ...
  function collectCoins(){
    for(let i=coins.length-1;i>=0;i--){
      ...
      if((nx-c.x)**2 + (ny-c.y)**2 <= c.r**2){ coins.splice(i,1); score+=100; beep(1200,80,'sine',0.25); }
    }
  }
  ...
  function collideEnemies(){
    ...
    const p = shrinkBox({x:player.x,y:player.y,w:player.w,h:player.h}, 0.1); // プレイヤーも少しゆるめ
    for(const e of enemies){
      const eb = shrinkBox(e, CONFIG.enemyHitboxShrink);
      if(hit(p, eb)){ lives -= CONFIG.enemyDamage; invincible = CONFIG.iFrames; flash=10; beep(200,120,'square',0.3); if(lives<=0) gameOver(); break; }
    }
  }
  ...
  function drawHUD(){
    const padY = padTop(), padR = padRight();
    const x = window.innerWidth - CONFIG.hudWidth - 12 - padR;
    const y = 8 + padY;
    // 背景（小さい角丸）
    roundRect(x,y,CONFIG.hudWidth,56,10,'var(--hud)');
    // テキスト
    ctx.fillStyle='var(--fg)'; ctx.font='700 18px monospace';
    ctx.fillText(`SCORE ${score|0}`, x+12, y+28);
    ctx.fillText(`LIFE  ${lives}`,  x+12, y+50);
    // 距離は右寄せ
    ctx.textAlign='right'; ctx.fillText(`${(distance|0)} m`, x+CONFIG.hudWidth-10, y+28); ctx.textAlign='left';
  }
  ...
  function drawHoles(){
    ...
    for(const h of holes){
      ...
      ctx.fillRect(h.x-6, GY()-6, 6, 6);
      ctx.fillRect(h.x+h.w, GY()-6, 6, 6);
      // 斜線
      ctx.save(); ctx.translate(h.x-6, GY()-6); ctx.rotate(Math.PI/6);
      ctx.fillRect(0,0,h.w+12,4);
      ctx.restore();
    }
  }
  function drawCoins(){
  ...
  function drawEnemies(){ for(const e of enemies){ if(images.enemy) ctx.drawImage(images.enemy,e.x,e.y,e.w,e.h); else{ ctx.fillStyle='#333'; ctx.fillRect(e.x,e.y,e.w,e.h);} } }
  ...
  function loop(ts){
    ...
    if(state==='home'){
      ...
      distance += speed*0.25; score += speed*0.35;
      updateWorld();
      ...
      if(flash>0){ ctx.fillStyle='rgba(255,0,0,.22)'; ctx.fillRect(0,0,window.innerWidth,window.innerHeight); flash--; }
